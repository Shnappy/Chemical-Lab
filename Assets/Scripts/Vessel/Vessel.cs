...
public class Vessel : MonoBehaviour
{
    ...
    private void InstantiateListedLiquids()
    {
        ...
        foreach (var liquid in liquids)
        {
            ...
            var liquidInstance = Instantiate(liquidPrefab, new Vector3(0.0f, - 40.0f, 0.0f), Quaternion.identity); //TODO come back to this when multi-level liquids are implemented
    ...
    private List<ChemicalEnums.Chemical> GetChemicalsInVessel(ChemicalComponent[] chemicalComponents)
    {
        List<ChemicalEnums.Chemical> chemicals = new List<ChemicalEnums.Chemical>();
        foreach (var chemicalComponent in chemicalComponents)
        {
            chemicals.Add(chemicalComponent._enum);
        }
        return chemicals; 
    }
}